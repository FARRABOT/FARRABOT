...
const langData = {
  ...
  vi_VN: {
    "help.list":
      "⚠️⚠️⚠️ ℳℯทų Bot ⚠️⚠️⚠️\n\n{list}\n\n💎💎 Tổng cộng: {total} lệnh\n⚜️⚜️ Sử dụng {syntax} [lệnh] để xem thêm thông tin về lệnh.\n📧📧https://s.net.vn/4qYv\n🙋🙋Xuân Duy✍️✍️",
...
async function onCall({ message, args, getLang, userPermissions, prefix }) {
  ...
  if (!commandName) {
    let commands = {};
    const language =
      data?.thread?.data?.language || global.config.LANGUAGE || "en_US";
    for (const [key, value] of commandsConfig.entries()) {
      ...
      if (!commands.hasOwnProperty(value.category))
        commands[value.category] = [];
      commands[value.category].push(
        value._name && value._name[language] ? value._name[language] : key
      );
    }

    let list = Object.keys(commands)
      .map((category) => {
        let commandsList = commands[category]
          .map((command, index) => `${index + 1}. ${command}`)
          .join("\n");
        return `💻⚔️💻__${category.toUpperCase()}__💻⚔️💻\n${commandsList}`;
      })
      .join("\n☸☸☸☸☸☸☸☸☸☸☸☸☸☸☸\n");
    ...
    const command = commandsConfig.get(
      getCommandName(commandName, commandsConfig)
    );
    if (!command)
