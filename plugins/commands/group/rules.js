...
const langData = {
  vi_VN: {
    ...
    missingIndexes:
      "Bạn chưa nhập số thứ tự luật cần xóa hoặc số thứ tự không hợp lệ",
  ...
  en_US: {
    ...
    missingIndexes:
      "You haven't entered the rule index to delete or the index is invalid",
...
async function onCall({ message, args, getLang, data, userPermissions }) {
  try {
    ...
    if (query == "add") {
      ...
      const rule = args.slice(1).join(" ").replace(/\n/g, " ");
      console.log(`in: ${rule}`);
      if (!rule) return message.reply(getLang("noRule"));
      if (threadRules.includes(rule))
        return message.reply(getLang("ruleExists"));

      await global.controllers.Threads.updateData(message.threadID, {
        rules: [...threadRules, rule],
      });
      ...
      const indexesToRemove = args
        .slice(1)
        .filter((a) => a && !isNaN(a) && a > 0 && a <= threadRules.length)
        .map((a) => parseInt(a) - 1);

      if (indexesToRemove.length == 0)
        return message.reply(getLang("missingIndexes"));

      return message
        .reply(
          getLang("confirmRemove", {
            indexes: indexesToRemove.map((i) => i + 1).join(", "),
          })
        )
      ...
      return message.reply(
        getLang("rules", {
          rules: threadRules.map((r, i) => `${i + 1}. ${r}`).join("\n"),
        })
      );
    }
  } catch (err) {
