          ...
          await message
            .send({
              body: getLang(_kick ? "kickSuccess" : "kickFail", {
                targetName,
                warns: (
                  members.find((member) => member.userID == targetID)?.warns ||
                  []
                )
                  .map((warn) => `• ${warn.reason} (${warn.time})`)
                  .join("\n"),
              }),
...
async function onCall({ message, args, getLang, data }) {
  try {
    ...
    const targetIDs = (
      messageReply?.senderID ? [messageReply.senderID] : Object.keys(mentions)
      ...
      return message.reply(
        getLang("warns", {
          warns: (data.thread.info.members || [])
            ...
            .map((member) => {
              let username =
                global.data.users.get(member.userID)?.info?.name ||
                member.userID;
